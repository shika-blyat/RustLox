...
impl TokenList {
    ...
    pub fn tokenize(&mut self) -> &Vec<Token> {
        ...
        for (k, i) in self.program.chars().enumerate() {
            ...
            if i.is_digit(10) || i == '.' && !is_identifier{
                if last_char.is_digit(10) || last_char == '.' && !is_identifier {
                    self.nbr_char += 1;
                } else {
                    looked_index = k;
                }
            ...
            } else if last_char.is_ascii_alphabetic() && is_identifier {
                    self.token_list.push(Token {
                    token_type: TokenType::Identifier(String::from(&self.program[looked_index..k])),
                    line: self.line,
                    nbr_char: self.nbr_char,
                });
                is_identifier = false;
